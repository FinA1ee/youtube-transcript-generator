...
The system SHALL render an English-language HTML page where users can submit a YouTube URL and see current generation status without navigating away.

#### Scenario: Page loads
- **WHEN** a user opens the application root page
- **THEN** the system displays a YouTube URL input, a submit control, and an empty report area

#### Scenario: Generation status updates
- **WHEN** the browser receives progress events
- **THEN** the page updates the visible English status for the current generation step
...
- **WHEN** the browser receives state or progress events for validation, Gemini setup preflight, caption fetching, transcript preparation, Gemini generation, streaming, completion, retry, or errors
...
- **WHEN** the browser receives an error event
- **THEN** the page displays a sanitized error message and allows the user to submit another URL

#### Scenario: Gemini setup check fails
- **WHEN** the browser receives a provider error event caused by Gemini setup preflight failure
- **THEN** the page displays an English notification that Gemini setup or calling failed
- **AND** allows the user to re-enter a URL without rendering report content
...
- **WHEN** the browser cannot restore the SSE stream after configured retries
...
- **THEN** the page closes the active SSE connection, preserves already rendered content, and returns the input controls to an idle state
